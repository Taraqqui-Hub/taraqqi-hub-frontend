...
const getStatusConfig = (t: (key: string) => string): Record<string, { label: string; color: string; bg: string; border: string; description: string }> => ({
	pending: { label: t("common.applied"), color: "text-blue-600", bg: "bg-blue-50", border: "border-blue-200", description: t("applications.applicationSubmitted") },
	reviewed: { label: t("applications.viewed"), color: "text-purple-600", bg: "bg-purple-50", border: "border-purple-200", description: t("applications.profileViewed") },
	shortlisted: { label: t("applications.shortlisted"), color: "text-amber-600", bg: "bg-amber-50", border: "border-amber-200", description: t("applications.youAreShortlisted") },
	interview: { label: t("applications.interview"), color: "text-indigo-600", bg: "bg-indigo-50", border: "border-indigo-200", description: t("applications.interviewScheduled") },
	offered: { label: t("applications.offer"), color: "text-emerald-600", bg: "bg-emerald-50", border: "border-emerald-200", description: t("applications.offerReceived") },
	hired: { label: t("applications.hired"), color: "text-green-600", bg: "bg-green-50", border: "border-green-200", description: t("applications.youAreHired") },
	rejected: { label: t("applications.notSelected"), color: "text-red-600", bg: "bg-red-50", border: "border-red-200", description: t("applications.applicationDeclined") },
	withdrawn: { label: t("applications.withdrawn"), color: "text-slate-500", bg: "bg-slate-100", border: "border-slate-200", description: t("applications.youWithdrew") },
});
...
export default function ApplicationsPage() {
	...
	const formatSalary = (min: string | null, max: string | null) => {
		...
		if (maxVal) return `${t("common.upTo")} ₹${format(maxVal)}`;
		return t("common.notSpecified");
	};

	const getStatusTimeline = (status: string) => {
		const steps = [
			{ key: "pending", label: t("common.applied") },
			{ key: "reviewed", label: t("applications.viewed") },
			{ key: "shortlisted", label: t("applications.shortlisted") },
			{ key: "interview", label: t("applications.interview") },
			{ key: "offered", label: t("applications.offer") },
			{ key: "hired", label: t("applications.hired") },
		];
			...
			<DashboardLayout>
				<div className="max-w-5xl mx-auto px-4 py-8">
					...
					<div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
						<div>
							<h1 className="text-2xl font-bold text-slate-900 tracking-tight">{t("applications.myApplications")}</h1>
							<p className="text-slate-500 mt-1">{t("dashboard.trackProgress")}</p>
						</div>
					...
					<div className="mb-8 overflow-x-auto pb-2 scrollbar-hide">
						<div className="flex items-center gap-2">
							<button
								...
								`}
							>
								{t("applications.all")} <span className="ml-1 opacity-60 text-xs">({applications.length})</span>
							</button>
					...
					{loading ? (
						<div className="text-center py-20">
							...
							<p className="mt-4 text-slate-500 text-sm">{t("applications.loadingApplications")}</p>
						</div>
										...
										<div className="p-5 sm:p-6">
											...
											<div className="flex flex-wrap items-center justify-between gap-4 pt-4 border-t border-slate-100">
												<div className="text-xs text-slate-400 font-medium flex items-center gap-1">
													...
													{t("applications.appliedOn")} {new Date(app.appliedAt).toLocaleDateString(undefined, {
														year: 'numeric',
														month: 'short',
														day: 'numeric'
													})}
												...
												<div className="flex items-center gap-3">
													{canWithdraw(app.status) && (
														...
														>
															{withdrawing === app.id ? t("common.loading") : t("applications.withdraw")}
														</button>
													)}
													<button
													...
													>
														{isExpanded ? t("common.lessDetails") : t("common.viewDetails")}
										...
										{isExpanded && (
											<div className="bg-slate-50 border-t border-slate-100 p-5 sm:p-6 animate-in slide-in-from-top-2 duration-200">
												<div className="grid md:grid-cols-2 gap-6">
													{app.coverLetter && (
														<div>
															<h4 className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-3 flex items-center gap-2">
																<FileText className="w-3 h-3" /> {t("applications.coverLetter")}
															</h4>
													...
													<div>
														<h4 className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-3 flex items-center gap-2">
															<CheckCircle2 className="w-3 h-3" /> {t("applications.applicationDetails")}
														</h4>
														<div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
															{app.expectedSalary && (
																<div className="flex justify-between p-3 border-b border-slate-50 last:border-0 hover:bg-slate-50/50 transition-colors">
																	<span className="text-sm text-slate-500">{t("applications.expectedSalaryLabel")}</span>
																	<span className="text-sm font-semibold text-slate-900">₹{app.expectedSalary}</span>
																</div>
															)}
															{app.noticePeriodDays !== null && (
																<div className="flex justify-between p-3 border-b border-slate-50 last:border-0 hover:bg-slate-50/50 transition-colors">
																	<span className="text-sm text-slate-500">{t("applications.noticePeriodLabel")}</span>
																	<span className="text-sm font-semibold text-slate-900">{app.noticePeriodDays} Days</span>
																</div>
															)}
															<div className="flex justify-between p-3 border-b border-slate-50 last:border-0 hover:bg-slate-50/50 transition-colors items-center">
																<span className="text-sm text-slate-500">{t("applications.resumeLabel")}</span>
																{app.job?.isResumeRequired === false ? (
																	<span className="text-sm text-slate-500">{t("applications.resumeNotRequired")}</span>
																) : app.resumeUrl ? (
																	<a
																		href={app.resumeUrl}
																		target="_blank"
																		rel="noopener noreferrer"
																		className="text-sm font-semibold text-indigo-600 hover:underline inline-flex items-center gap-1"
																	>
																		<FileText className="w-3.5 h-3.5" /> {t("applications.viewResume")}
																	</a>
																) : (
																	<span className="text-sm text-slate-500">{t("applications.noResumeAttached")}</span>
																)}
															</div>
														</div>
													</div>
